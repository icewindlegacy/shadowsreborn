...
void nanny (DESCRIPTOR_DATA * d, char *argument)
{
    ...
    switch (d->connected)
    {
        ...
        case CON_ANSI:
            ...
            {
                send_to_desc ("Does your terminal support color? ", d);
                return;
            }
...
/* RT code for breaking link */
        ...
        case CON_CONFIRM_NEW_PASSWORD:
            ...
            write_to_buffer (d, "\n\r", 0);
            send_to_desc ("Orcish Races:\n\r  ", d);
            for (race = 13; race_table[race].name != NULL && race <= 24; race++)
            {
                if (race_table[race].pc_race)
                {
                    write_to_buffer (d, race_table[race].name, 0);
        ...
        case CON_GET_NEW_RACE:
            ...
            ch->race = race;
            /* assign faction based on race */
            ch->faction = get_faction_by_race(race);
            /* initialize stats */
        ...
        case CON_GET_NEW_SEX:
            ...
            strcpy (buf, "Select a class [");
            {
                ...
                /* Determine which classes to show based on race faction */
                if (ch->race >= 1 && ch->race <= 12)
                {
                    /* Human faction races: show human classes (0-3) */
                    class_start = 0;
                    class_end = 3;
                }
                else if (ch->race >= 13 && ch->race <= 24)
                {
                    /* Orcish faction races: show orcish classes (4-7) */
                    class_start = 4;
                    class_end = 7;
                }
                else
                {
                    ...
                    class_start = 0;
                    class_end = MAX_CLASS - 1;
                }
                
                for (iClass = class_start; iClass <= class_end; iClass++)
                {
                    if (!first)
                        strcat (buf, " ");
        ...
        case CON_GET_NEW_CLASS:
            ...
            /* Validate that the class matches the race's faction */
            {
                int class_start, class_end;
                bool valid_class = FALSE;
                
                /* Determine valid class range based on race faction */
                if (ch->race >= 1 && ch->race <= 12)
                {
                    /* Human faction races: only allow human classes (0-3) */
                    class_start = 0;
                    class_end = 3;
                }
                else if (ch->race >= 13 && ch->race <= 24)
                {
                    /* Orcish faction races: only allow orcish classes (4-7) */
                    class_start = 4;
                    class_end = 7;
                }
                else
                {
                    /* Fallback: allow all classes */
                    class_start = 0;
                    class_end = MAX_CLASS - 1;
                }
                
                if (iClass >= class_start && iClass <= class_end)
                    valid_class = TRUE;
                
                if (!valid_class)
                {
                    send_to_desc ("That class is not available for your race.\n\rWhat IS your class? ",
                                  d);
                    return;
                }
            }
        ...
        case CON_GET_ALIGNMENT:
            ...
            /* Assign languages based on race */
            ...
            {
                ch->pcdata->learned[gsn_elvish] = 100;
            }
            else if (ch->race == race_lookup("dwarf"))
            {
                ch->pcdata->learned[gsn_dwarvish] = 100;
            }
            else if (ch->race == race_lookup("verbeeg"))
            {
                ch->pcdata->learned[gsn_verbeeg] = 100;
            }
        ...
        case CON_DEFAULT_CHOICE:
            ...
            switch (argument[0])
            {
                ...
                case 'N':
                    ...
                    /*
                     * Add class-specific skills available at level 1.
                     * This is especially important for orcish classes like berserker
                     * that share default groups with their human counterparts but have
                     * unique skills (e.g., berserk) that aren't in the shared group.
                     */
                    {
                        int sn;
                        for (sn = 0; sn < MAX_SKILL; sn++)
                        {
                            ...
                            /* Skip if already learned or if it's a spell */
                            if (ch->pcdata->learned[sn] > 0
                                || skill_table[sn].spell_fun != spell_null)
                                continue;
                            
                            /* Add skills that are available at level 1 for this class */
                            if (skill_table[sn].skill_level[ch->class] == 1)
                            {
                                /* Only add if it's not already in a group we added */
        ...
        case CON_GEN_GROUPS:
            ...
            {
                int racial_points = pc_race_table[ch->race].points;
                int points_spent = ch->pcdata->points - racial_points;
                
                sprintf (buf, "Creation points: %d (racial: %d, spent: %d)\n\r", 
                         ch->pcdata->points, racial_points, points_spent);
                send_to_char (buf, ch);
                sprintf (buf, "Experience per level: %d\n\r",
                         exp_per_level (ch, ch->gen_data->points_chosen));
                send_to_char (buf, ch);
                
                /* Calculate trains based on creation points spent */
                if (points_spent == 0)
                {
                    /* No skills/spells taken, only racial = 10 trains */
                    ch->train = 10;
                    sprintf (buf, "You will start with %d trains.\n\r", ch->train);
                    send_to_char (buf, ch);
                }
                else if (points_spent >= 10 && points_spent <= 19)
                {
                    ch->train = 8;
                    sprintf (buf, "You will start with %d trains.\n\r", ch->train);
                    send_to_char (buf, ch);
                }
                else if (points_spent >= 20 && points_spent <= 29)
                {
                    ch->train = 6;
                    sprintf (buf, "You will start with %d trains.\n\r", ch->train);
                    send_to_char (buf, ch);
                }
                else if (points_spent >= 30 && points_spent <= 39)
                {
                    ch->train = 4;
                    sprintf (buf, "You will start with %d trains.\n\r", ch->train);
                    send_to_char (buf, ch);
                }
                else if (points_spent >= 40)
                {
                    ch->train = 3;
                }
                else
                {
                    /* For 1-9 points, give bonus trains (interpolate between 10 and 8) */
                    ch->train = 10 - (points_spent * 2 / 10);
                    sprintf (buf, "You will start with %d trains.\n\r", ch->train);
                    send_to_char (buf, ch);
                }
		...
		if(mud_telnetga)
			SET_BIT (ch->comm, COMM_TELNET_GA);
                ...
                ch->mana = ch->max_mana;
                ch->move = ch->max_move;
                /* Only set default trains if not already set by customization */
                if (ch->train == 0)
                    ch->train = 3;
                ch->practice = 5;
                sprintf (buf, "the FNG");
                ...
                obj_to_char (create_object (get_obj_index (OBJ_VNUM_MAP), 0),
                    ...
                    obj_to_char (create_object (get_obj_index (OBJ_VNUM_ORC_MAP), 0),
                             ch);

                /* Place character in faction-appropriate mud school */
                if (ch->faction == FACTION_HUMAN)
