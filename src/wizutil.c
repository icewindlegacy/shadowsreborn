 ...
 *           wizutil.c - November 13, 2025
 */
/***************************************************************************
 *  Original Diku Mud copyright (C) 1990, 1991 by Sebastian Hammer,        *
 *  Michael Seifert, Hans Henrik Stärfeldt, Tom Madsen, and Katja Nyboe.   *
 *                                                                         *
...
bool_t check_parse_name (char* name);  /* comm.c */

/* Get the first letter of a name for directory structure */
...
const char * name_expand (CHAR_DATA *ch)
{
	...
	snprintf (outbuf, sizeof(outbuf), "%d.%.251s", count, name);
	return outbuf;
}
...
void do_for (CHAR_DATA *ch, char *argument)
{
	...
	char buf[MAX_STRING_LENGTH];
	bool_t fGods = FALSE, fMortals = FALSE, fMobs = FALSE, fEverywhere = FALSE, found;
	...
	{
		send_to_char ("Syntax: FOR <all|gods|mortals|mobs|everywhere> <command>\n\r", ch);
		return;
	}
