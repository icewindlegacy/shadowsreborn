...
int get_weapon_sn (CHAR_DATA * ch)
{
    ...
    {
        /* Check for ohshit skill - if holding an object and have the skill */
        if (!IS_NPC (ch) && get_eq_char (ch, WEAR_HOLD) != NULL)
        {
            if (get_skill (ch, gsn_ohshit) > 0)
                sn = gsn_ohshit;
            else
                sn = gsn_hand_to_hand;
        }
...
int get_obj_max_condition (const OBJ_DATA *obj)
{
    int max_cond;
    ...
    if (obj->pIndexData != NULL && obj->pIndexData->condition > 0)
        max_cond = obj->pIndexData->condition;
    else
        max_cond = 100;
...
void damage_item_condition (OBJ_DATA *obj, int amount, CHAR_DATA *owner)
{
    int max_cond;
    int old_cond;
    int new_cond;
    ...
    old_cond = obj->condition;
    obj->condition = UMAX(0, obj->condition - amount);
    new_cond = obj->condition;
    ...
    old_percent = (old_cond * 100) / max_cond;
    new_percent = (new_cond * 100) / max_cond;

    if (owner != NULL)
    {
        if (!IS_NPC(owner) || owner->in_room != NULL)
        {
            if (old_percent > 75 && new_percent <= 75)
                act("{Y$p shows signs of wear.{x", owner, obj, NULL, TO_CHAR);
            else if (old_percent > 50 && new_percent <= 50)
                act("{Y$p looks worn from use.{x", owner, obj, NULL, TO_CHAR);
            else if (old_percent > 25 && new_percent <= 25)
                act("{R$p is badly damaged!{x", owner, obj, NULL, TO_CHAR);
            else if (old_percent > 10 && new_percent <= 10)
                act("{R$p is barely holding together!{x", owner, obj, NULL, TO_CHAR);
            else if (new_percent == 0 && old_percent > 0)
            {
                act("{R$p is completely ruined and falls apart!{x", owner, obj, NULL, TO_CHAR);
                act("{R$n's $p is completely ruined and falls apart!{x", owner, obj, NULL, TO_ROOM);
                
                /* Auto-unequip items at 0% condition */
                if (obj->wear_loc != WEAR_NONE)
                {
                    unequip_char(owner, obj);
                    act("{RYou can't use $p anymore until it's repaired!{x", owner, obj, NULL, TO_CHAR);
                }
            }
        }
    }
...
void obj_from_room (OBJ_DATA * obj)
{
    ...
    /* If this was a light item, it will no longer be counted by count_room_lights() */
    return;
}
...
void obj_to_room (OBJ_DATA * obj, ROOM_INDEX_DATA * pRoomIndex)
{
    ...
    /* If this is a light item, it will be counted by count_room_lights() */
    return;
}
...
void obj_to_obj (OBJ_DATA * obj, OBJ_DATA * obj_to)
{
    ...
    {
        obj->cost = 0;
        save_pit(obj_to);  /* Save pit when item added */
    }
    else if (obj_to->pIndexData->vnum == OBJ_VNUM_ORC_PIT)
    {
        obj->cost = 0;
        save_pit(obj_to);  /* Save orc pit when item added */
    }
...
{
    /* Safety check for NULL room */
    if (pRoomIndex == NULL)
        return TRUE;
    
    /* Check for light from room light count or light items in room */
    if (pRoomIndex->light > 0 || count_room_lights(pRoomIndex) > 0)
        return FALSE;
...
/*
 * Count light sources in a room (items on ground and worn by characters)
 */
int count_room_lights (ROOM_INDEX_DATA * pRoomIndex)
{
    OBJ_DATA *obj;
    CHAR_DATA *ch;
    int light_count = 0;
    ...
    /* Count light items on the ground */
    ...
    /* Count light items worn by characters in the room */
...
bool can_see_room (CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex)
{
    ...
    /* Don't block room entry/exits for magical darkness - just vision */
...
/* RT changed so that WIZ_INVIS has levels */
    ...
    /* Check for magical darkness - only drow can see through it */
    if (ch->in_room != NULL && IS_SET(ch->in_room->room_flags, ROOM_MALEDICTION))
    {
        if (!IS_IMMORTAL(ch) && (IS_NPC(ch) || ch->race != race_lookup("drow")))
            return FALSE;
    }
...
char *act_bit_name (int act_flags)
{
    ...
    if (IS_SET (act_flags, ACT_IS_NPC))
    {
        ...
        if (act_flags & ACT_BOUNTY)
            strcat (buf, " bounty");
        if (act_flags & ACT_ARMORSMITH)
            strcat (buf, " armorsmith");
    }
    else
    {
        strcat (buf, " player");
...
char *gun_bit_name (int gun_flags)
{
    static char buf[512];

    buf[0] = '\0';
    if (gun_flags & GUN_CLIP) strcat(buf, " clip");
    if (gun_flags & GUN_ENERGY) strcat(buf, " energy");
    if (gun_flags & GUN_FUEL) strcat(buf, " fuel");
    if (gun_flags & GUN_SHELL) strcat(buf, " shell");
    if (gun_flags & GUN_ROCKET) strcat(buf, " rocket");
    if (gun_flags & GUN_DUAL) strcat(buf, " dual");
    if (gun_flags & GUN_BURST) strcat(buf, " burst");
    if (gun_flags & GUN_AUTO) strcat(buf, " auto");
    if (gun_flags & GUN_SINGLE) strcat(buf, " single");
    if (gun_flags & GUN_RECOIL_LITTLE) strcat(buf, " little");
    if (gun_flags & GUN_RECOIL_NORMAL) strcat(buf, " normal");
    if (gun_flags & GUN_RECOIL_HARD) strcat(buf, " hard");
    if (gun_flags & GUN_RECOIL_SEVERE) strcat(buf, " severe");
...
/* Fallen Angels auto_quaff function */
