 ...
 */
#define            MAX_STRING    4194304  /* Increased from 1.35MB to 4MB to accommodate larger area data */
...
void boot_db ()
{
     ...
     */
    {
        /* Free existing string_space if it exists (shouldn't happen, but be safe) */
        ...
        /* Reset string hash table to clear any stale pointers */
        memset (string_hash, 0, sizeof (string_hash));
        
        /* Reset string allocation counters */
        nAllocString = 0;
        sAllocString = 0;
     ...
     */
    {
        ...
        /* Initialize the new weather system */
        init_weather();
...
void load_area (FILE * fp)
{
	...
	mud_logf("Loading area %s", pArea->file_name);
...
void new_load_area (FILE * fp)
{
    AREA_DATA *pArea;
    char *word;
    bool fMatch __attribute__((unused));
    ...
    for (;;)
    {
        ...
        switch (UPPER (word[0]))
        {
            ...
            case 'E':
                if (!str_cmp (word, "End"))
                {
                    fMatch = TRUE; /* Set by macro convention, but not used */
                    if (area_first == NULL)
                        area_first = pArea;
                    if (area_last != NULL)
                        area_last->next = pArea;
...
void load_mobprogs (FILE * fp)
{
	...
	mud_logf("Loaded %d mobprogs.", top_mprog_index);
    return;
}
                    ...
                    if (pObjIndex->count < limit || number_range (0, 4) == 0)
                    {
                        ...
                        if (pObj->level > LastMob->level + 3
                            || (pObj->item_type == ITEM_WEAPON
                                ...
                                && pObj->level < 45)) {
                            mud_logf ("Check Levels:");
							mud_logf ("  Object: (VNUM %5d)(Level %2d) %s",
                                     pObj->pIndexData->vnum,
									 pObj->level,
                                     pObj->short_descr);
							mud_logf ("     Mob: (VNUM %5d)(Level %2d) %s",
...
OBJ_DATA *create_object (OBJ_INDEX_DATA * pObjIndex, int level)
{
    ...
    /* Use -1 to mean "not set" instead of 0, since 0 is now a valid condition (broken) */
    obj->condition = (pObjIndex->condition >= 0) ? pObjIndex->condition : 100;
    ...
    /* Initialize object leveling fields */
    obj->exp = pObjIndex->exp;
    obj->plevel = pObjIndex->plevel;
    obj->xp_tolevel = pObjIndex->xp_tolevel;
...
char *fread_string (FILE * fp)
{
    ...
    /* Safety check: ensure top_string is valid */
    if (string_space == NULL || top_string == NULL)
    {
        bug ("Fread_string: string_space or top_string is NULL.", 0);
        exit (1);
    }
    
    if (top_string < string_space || top_string >= string_space + MAX_STRING)
    {
        bugf ("Fread_string: top_string (%p) is outside string_space (%p - %p).", 
              top_string, string_space, string_space + MAX_STRING);
        exit (1);
    }
    ...
    {
        bugf ("Fread_string: MAX_STRING %d exceeded. top_string offset: %ld, max allowed: %ld", 
              MAX_STRING, (long)(top_string - string_space), 
              (long)(MAX_STRING - MAX_STRING_LENGTH - sizeof(char*)));
        exit (1);
    }
...
/* Load corpses from file */
void load_corpses( void )
{
    FILE *corpsefp;
    char corpse_file_name[MAX_STRING_LENGTH];

    sprintf( corpse_file_name, "../area/%s", CORPSE_FILE );

    if ( ( corpsefp = fopen( corpse_file_name, "r" ) ) == NULL )
    {
        bug( "Load corpse Table: fopen", 0 );
        perror( "failed open of corpses.lst in load_corpses" );
        return;
    }
...
/* Read a corpse object from file */
        ...
        case 'E':
            ...
            if ( !str_cmp( word, "End" ) )
            {
                ...
                else
                {
                    ...
                    /* Add to corpse list */
                    corpse = alloc_mem(sizeof(*corpse));
                    corpse->this_corpse = obj;
                    corpse->next = first_corpse;
                    first_corpse = corpse;
...
/*
 * Load donation pit contents
 */
void load_pit( int vnum )
{
    char filename[256];
    FILE *fp;
    OBJ_DATA *pit = NULL;
    ...
    sprintf( filename, "%spit_%d.vault", STORAGE_DIR, vnum );

    if ( ( fp = fopen( filename, "r" ) ) != NULL )
    {
        log_string( filename );
        
        /* Find the pit object in the world */
        for ( obj = object_list; obj != NULL; obj = obj->next )
        {
        ...
        if ( !pit )
        {
            bugf( "load_pit: couldn't find pit vnum %d in game", vnum );
            fclose( fp );
            return;
        }
        
        /* Clear any existing objects in the pit before loading */
        while (pit->contains != NULL)
        {
            extract_obj(pit->contains);
        }
        ...
        for ( ; ; )
        {
            ...
            if ( letter != '#' )
            {
                bugf( "Load_pit: # not found in %s.", filename );
                break;
            }

            word = fread_word( fp );
            if ( !str_cmp( word, "OBJECT" ) || !str_cmp( word, "OBJECTS" ) || !str_cmp( word, "O" ) )
            {
                /* Create a temporary character to load object to */
                CHAR_DATA temp_ch;
                memset(&temp_ch, 0, sizeof(CHAR_DATA));
                temp_ch.in_room = NULL;
                
                fread_obj( &temp_ch, fp );
                
                /* Move all loaded objects from temp char to pit */
                while (temp_ch.carrying != NULL)
                {
            ...
            {
                int file_vnum = fread_number( fp );
                if ( file_vnum != vnum )
                {
                    bugf( "load_pit: vnum mismatch - expected %d, got %d in %s.", 
                          vnum, file_vnum, filename );
                }
            }
            else if ( !str_cmp( word, "END"    ) )
                break;
            ...
            {
                bugf( "Load_pit: bad section '%s' in %s.", word, filename );
                break;
            }
        }
        fclose( fp );
...
/*
 * Load all donation pits
 */
void load_pits( void )
{
    load_pit( OBJ_VNUM_PIT );
    load_pit( OBJ_VNUM_ORC_PIT );
}
...
/*
 * Load all clan storage rooms
 * Adapted from SMAUG clanvault snippet by Rhys
 */
#include <dirent.h>
...
void load_storages( void )
{
    ...
    sprintf(dirname, "%s", STORAGE_DIR);
    dp = opendir( dirname );
    
    if (dp == NULL)
    {
        bug("load_storages: cannot open storage directory", 0);
        return;
    }
...
void load_storage( char *filename )
{
    ...
    if ( ( fp = fopen( buf, "r" ) ) != NULL )
    {
        ...
        for ( ; ; )
        {
            ...
            if ( letter != '#' )
            {
                bugf( "Load_storage: # not found in %s.", filename );
                break;
            }
            ...
            if ( !str_cmp( word, "OBJECT" ) || !str_cmp( word, "OBJECTS" ) || !str_cmp( word, "O" ) )
            {
                if (room != NULL)
                {
                    ...
                    CHAR_DATA temp_ch;
                    memset(&temp_ch, 0, sizeof(CHAR_DATA));
                    temp_ch.in_room = room;
                    
                    fread_obj( &temp_ch, fp );
                    
                    /* Move all loaded objects from temp char to room */
                    while (temp_ch.carrying != NULL)
                    {
                        obj = temp_ch.carrying;
                        obj_from_char(obj);
                ...
                {
                    bugf( "Load_storage: No room set yet in %s - skipping object", filename );
                    /* fread_obj will handle the skipping */
                    fread_obj(NULL, fp);
                }
            }
            else if( !str_cmp( word, "VNUM" ) )
            {
                ...
                /* Clear any existing objects in the storeroom before loading */
            ...
            {
                bugf( "Load_storage: bad section '%s' in %s.", word, filename );
                break;
            }
        }
        fclose( fp );
