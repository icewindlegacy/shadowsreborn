...
void save_traps(void)
{
    ...
    sprintf(filename, "%straps.dat", STORAGE_DIR);
    
    if ((fp = fopen(filename, "w")) == NULL)
    {
        bug("save_traps: fopen", 0);
        perror(filename);
        return;
    }
    
    fprintf(fp, "* Trap Data File - Auto-generated\n");
    fprintf(fp, "* Format: vnum room_vnum trap_eff trap_dam trap_charge trap_enabled\n\n");
    
    for (obj = object_list; obj != NULL; obj = obj->next)
    {
        /* Only save traps that are in rooms (not carried or in containers) */
        if (IS_SET(obj->extra_flags, ITEM_TRAP) && obj->in_room != NULL && obj->in_obj == NULL)
        {
    ...
    log_string("Trap data saved.");
}

/*
 * Load all trap data from file
 */
void load_traps(void)
{
    FILE *fp;
    char filename[MAX_STRING_LENGTH];
    int vnum, room_vnum, trap_eff, trap_dam, trap_charge, trap_enabled;
    int count = 0;
    ...
    if ((fp = fopen(filename, "r")) == NULL)
    {
        ...
        log_string("No trap data file found - starting fresh.");
        return;
    }
    ...
    for (;;)
    {
        ...
        trap_eff = fread_number(fp);
        trap_dam = fread_number(fp);
        trap_charge = fread_number(fp);
        trap_enabled = fread_number(fp);
        
        /* Find the trap object in the room */
        if ((room = get_room_index(room_vnum)) == NULL)
        {
            bugf("load_traps: Invalid room vnum %d for trap vnum %d", room_vnum, vnum);
            continue;
        }
        
        /* Find the specific trap object in the room */
        for (obj = room->contents; obj != NULL; obj = obj->next_content)
        {
            if (obj->pIndexData->vnum == vnum && IS_SET(obj->extra_flags, ITEM_TRAP))
            {
                obj->trap_eff = trap_eff;
                obj->trap_dam = trap_dam;
                obj->trap_charge = trap_charge;
                obj->trap_enabled = (trap_enabled != 0);
                count++;
                break;
            }
        }
    ...
    {
        char buf[MAX_STRING_LENGTH];
        sprintf(buf, "Loaded trap data for %d traps.", count);
        log_string(buf);
    }
}
