...
void do_outfit (CHAR_DATA * ch, char *argument)
{
    OBJ_DATA *obj;
    int i, sn, vnum;
    int banner_vnum, vest_vnum, shield_vnum, comm_vnum;
    ...
    if (ch->faction == FACTION_ORCISH)
    {
        banner_vnum = OBJ_VNUM_ORC_BANNER;
        vest_vnum = OBJ_VNUM_ORC_VEST;
        shield_vnum = OBJ_VNUM_ORC_SHIELD;
        comm_vnum = OBJ_VNUM_ORC_COMM;
    }
    else
    {
    ...
    if ((obj = get_eq_char (ch, WEAR_COMM)) == NULL)
    {
    ...
    if (ch->race != race_lookup ("felar") && (obj = get_eq_char (ch, WEAR_WIELD)) == NULL)
    {
        ...
        for (i = 0; weapon_table[i].name != NULL; i++)
        {
                ...
                ch->pcdata->learned[*weapon_table[i].gsn])
            {
                ...
                /* Map weapon_table vnum to faction-appropriate vnum */
                if (ch->faction == FACTION_ORCISH)
                {
                    switch (weapon_table[i].vnum)
                    {
                        case OBJ_VNUM_SCHOOL_SWORD:   vnum = OBJ_VNUM_ORC_SWORD; break;
                        case OBJ_VNUM_SCHOOL_MACE:    vnum = OBJ_VNUM_ORC_MACE; break;
                        case OBJ_VNUM_SCHOOL_DAGGER:  vnum = OBJ_VNUM_ORC_DAGGER; break;
                        case OBJ_VNUM_SCHOOL_AXE:     vnum = OBJ_VNUM_ORC_AXE; break;
                        case OBJ_VNUM_SCHOOL_STAFF:   vnum = OBJ_VNUM_ORC_STAFF; break;
                        case OBJ_VNUM_SCHOOL_FLAIL:   vnum = OBJ_VNUM_ORC_FLAIL; break;
                        case OBJ_VNUM_SCHOOL_WHIP:    vnum = OBJ_VNUM_ORC_WHIP; break;
                        case OBJ_VNUM_SCHOOL_POLEARM: vnum = OBJ_VNUM_ORC_POLEARM; break;
                        default: vnum = OBJ_VNUM_ORC_SWORD; break;
                    }
                }
                else
                {
...
void do_mstat (CHAR_DATA * ch, char *argument)
{
    ...
    {
        sprintf (buf,
                 "Thirst: %d  Hunger: %d  Full: %d  Drunk: %d  Bld: %d\n\r",
...
void do_mset (CHAR_DATA * ch, char *argument)
{
    ...
    if (arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0')
    {
        ...
        send_to_char ("    security hours bleeding\n\r", ch);
        return;
    }
...
/* Quick command to damage an item for testing */
void do_damage_item (CHAR_DATA * ch, char *argument)
{
    char arg1[MAX_INPUT_LENGTH];
    char arg2[MAX_INPUT_LENGTH];
    OBJ_DATA *obj;
    ...
    if (arg1[0] == '\0')
    {
        send_to_char ("Syntax: damage <object> [amount]\n\r", ch);
        send_to_char ("        Damages an object by the specified amount (default 10).\n\r", ch);
        return;
    }
    ...
    damage_item_condition (obj, amount, ch);
    send_to_char ("Item damaged.\n\r", ch);
    return;
}
...
"--------------------------------------------------------------------------\n\r");  
    for ( d = descriptor_list; d; d = d->next )
    {
        if ( d->character && can_see( ch, d->character ) )
        {
           ...
           vch = d->original ? d->original : d->character;
           {
               struct tm *tm_info = localtime( &vch->logon );
               strftime( s, 100, "%I:%M%p", tm_info );
           }
...
/* NOTE: CH, COPYOVER_FILE, and EXE_FILE are now defined in merc.h */
...
void do_copyover (CHAR_DATA * ch, char *argument)
{
   ...
   // Save all objects on the ground for copyover
{
    FILE *fpObj;
    OBJ_DATA *obj;
    RESET_DATA *pReset;
    ...
    if (fpObj)
    {
        ...
        {   
            ...
            // Skip objects in clan storerooms (they have their own vault files)
            if (IS_SET(obj->in_room->room_flags, ROOM_CLANSTOREROOM))
                continue;
            
            // Skip reset-based objects
            for (pReset = obj->in_room->reset_first; pReset; pReset = pReset->next)
            {   
                if (((pReset->command == 'O' || pReset->command == 'P') && pReset->arg1 == obj->pIndexData->vnum) ||
                    (pReset->command == 'E' && pReset->arg1 == obj->pIndexData->vnum))
                {   
                    sprintf(buf, "Copyover: Skipping reset object %s (vnum %d) in room %d", 
                            obj->short_descr, obj->pIndexData->vnum, obj->in_room->vnum);
                    bug(buf, 0);
                    is_reset_object = TRUE;
                    break;
                }
            }
...
void copyover_recover ()
{
    ...
    /* Load objects saved during copyover */
    load_copyover_obj ();
...
void load_copyover_obj (void)
{
    ...
    fpObj = fopen ("../area/objcopy.txt", "r");
    if (!fpObj)
        return;
        
    unlink ("../area/objcopy.txt"); // To prevent from chars doubling objects.
    mud_logf ("Loading copyover objects.");
...
void qmconfig_read (void) {
	...
	mud_logf("Loading configuration settings from qmconfig.rc...");

	fp = fopen("../area/qmconfig.rc","r");
	if (!fp) {
		mud_logf("qmconfig.rc not found. Using compiled-in defaults.");
		return;
	}
	...
	for(;;) {
		...
		if (!fMatch) {
			mud_logf("qmconfig_read: no match for %s!", word);
			fread_to_eol(fp);
		}
		mud_logf("Settings read");
	}	
  ...
  void do_nuke(CHAR_DATA *ch, char *argument)
{
   ...
   if (IS_IMMORTAL(victim))/* nuclear immortals is the job of the MUDs owner */
   {
   if ( !str_cmp(ch->name, "Om"))
   {
     send_to_char("Authorization granted.{/", ch);
     sprintf(buf, "Thank you %s, for using the {GU B Glass{x ver 1.0 nuclear system.{/", ch->name);
     send_to_char( buf, ch );
...
void do_plist( CHAR_DATA *ch, char *argument )
{
	 ...
	 */
	    snprintf( buf, sizeof(buf), "%s%s", fImmortal ? "../gods/" : "../player/", ep->d_name );
	 ...
	 */
	    snprintf( buf, sizeof(buf), "%-15.15s %s%-3d{x days\n\r",
...
void do_jail( CHAR_DATA *ch, char *argument )
{
    ...
    /* Transfer victim to jail room 1212 */
    char_from_room(victim);
    char_to_room(victim, get_room_index(1212));
    act("$n has been jailed!", victim, NULL, NULL, TO_ROOM);

    return;
}

void do_ispellup(CHAR_DATA *ch, char *argument)
{
 CHAR_DATA *vch;
  char arg[MAX_INPUT_LENGTH];
 DESCRIPTOR_DATA *d;
...
void do_fish(CHAR_DATA *ch, char *argument)
{
    OBJ_DATA *pole = NULL;
    OBJ_DATA *bait = NULL;
    OBJ_DATA *fish_obj = NULL;
    char arg[MAX_INPUT_LENGTH];
    int pole_quality = 0;
    int bait_quality = 0;
    int max_fish_quality = 0;
    int fish_quality = 0;
    int roll = 0;

    /* Check if player is in water or on a boat */
    if (ch->in_room->sector_type != SECT_WATER_SWIM && 
        ch->in_room->sector_type != SECT_WATER_NOSWIM)
    {
        send_to_char("You need to be in water to fish!\n\r", ch);
        return;
    }

    /* Find fishing pole in inventory */
    for (pole = ch->carrying; pole != NULL; pole = pole->next_content)
    {
        if (pole->item_type == ITEM_FISHING_POLE)
            break;
    }
    ...
    pole_quality = pole->value[0];
    if (pole_quality < 0) pole_quality = 0;
    if (pole_quality > 5) pole_quality = 5;

    /* Parse argument for bait */
    argument = one_argument(argument, arg);
    if (arg[0] != '\0')
    {
        /* Find specified bait in inventory */
        for (bait = ch->carrying; bait != NULL; bait = bait->next_content)
        {
            if (bait->item_type == ITEM_BAIT && 
                is_name(arg, bait->name))
                break;
        }

        if (bait == NULL)
        {
            send_to_char("You don't have that bait!\n\r", ch);
            return;
        }

        bait_quality = bait->value[0];
        if (bait_quality < 0) bait_quality = 0;
        if (bait_quality > 5) bait_quality = 5;
    }

    /* Calculate maximum fish quality */
    max_fish_quality = pole_quality;
    if (bait != NULL)
        max_fish_quality += bait_quality;

    if (max_fish_quality > 10)
        max_fish_quality = 10;

    /* Roll for fishing result */
    roll = number_range(1, 100);

    /* 10% chance for line to break */
    if (roll <= 10)
    {
        send_to_char("Your fishing line breaks! You'll need to repair it before fishing again.\n\r", ch);
        return;
    }

    /* 20% chance to catch nothing */
    if (roll <= 30)
    {
        send_to_char("You don't catch anything this time.\n\r", ch);
        return;
    }

    /* Determine fish quality */
    fish_quality = number_range(0, max_fish_quality);

    /* Create fish object */
    OBJ_INDEX_DATA *fish_index = get_obj_index(OBJ_VNUM_FISH);
    if (fish_index == NULL)
    {
        send_to_char("The fish object template doesn't exist yet. Please create a fish object with vnum 9 first.\\n\\r", ch);
        return;
    }
    fish_obj = create_object(fish_index, 0);
    if (fish_obj == NULL)
    {
        send_to_char("Something went wrong while creating the fish!\n\r", ch);
        return;
    }

    /* Set fish quality in value[0] */
    fish_obj->value[0] = fish_quality;

    /* Set fish name based on quality */
    char fish_name[256];
    switch (fish_quality)
    {
        case 0:
            ...
            {
                case 1: strcpy(fish_name, "rock"); break;
                case 2: strcpy(fish_name, "old shoe"); break;
                case 3: strcpy(fish_name, "bundle of reeds"); break;
            }
            break;
        case 1:
            switch (number_range(1, 3))
            {
                case 1: strcpy(fish_name, "tiny minnow"); break;
                case 2: strcpy(fish_name, "small guppy"); break;
                case 3: strcpy(fish_name, "little goldfish"); break;
            }
            break;
        case 2:
            switch (number_range(1, 3))
            {
                case 1: strcpy(fish_name, "small perch"); break;
                case 2: strcpy(fish_name, "little bass"); break;
                case 3: strcpy(fish_name, "tiny trout"); break;
            }
            break;
        case 3:
            switch (number_range(1, 3))
            {
                case 1: strcpy(fish_name, "perch"); break;
                case 2: strcpy(fish_name, "bass"); break;
                case 3: strcpy(fish_name, "trout"); break;
            }
            break;
        case 4:
            switch (number_range(1, 3))
            {
                case 1: strcpy(fish_name, "large perch"); break;
                case 2: strcpy(fish_name, "big bass"); break;
                case 3: strcpy(fish_name, "large trout"); break;
            }
            break;
        case 5:
            switch (number_range(1, 3))
            {
                case 1: strcpy(fish_name, "pike"); break;
                case 2: strcpy(fish_name, "walleye"); break;
                case 3: strcpy(fish_name, "catfish"); break;
            }
            break;
        case 6:
            switch (number_range(1, 3))
            {
                case 1: strcpy(fish_name, "salmon"); break;
                case 2: strcpy(fish_name, "tuna"); break;
                case 3: strcpy(fish_name, "cod"); break;
            }
            break;
        case 7:
            switch (number_range(1, 3))
            {
                case 1: strcpy(fish_name, "large salmon"); break;
                case 2: strcpy(fish_name, "big tuna"); break;
                case 3: strcpy(fish_name, "huge cod"); break;
            }
            break;
        case 8:
            switch (number_range(1, 3))
            {
                case 1: strcpy(fish_name, "swordfish"); break;
                case 2: strcpy(fish_name, "barracuda"); break;
                case 3: strcpy(fish_name, "sturgeon"); break;
            }
            break;
        case 9:
            switch (number_range(1, 3))
            {
                case 1: strcpy(fish_name, "giant swordfish"); break;
                case 2: strcpy(fish_name, "large barracuda"); break;
                case 3: strcpy(fish_name, "massive sturgeon"); break;
            }
            break;
        case 10:
            switch (number_range(1, 3))
            {
                case 1: strcpy(fish_name, "legendary marlin"); break;
                case 2: strcpy(fish_name, "giant shark"); break;
                case 3: strcpy(fish_name, "ancient sea monster"); break;
            }
            break;
    }

    /* Set short description and description */
    char short_buf[512], desc_buf[512];
    sprintf(short_buf, "a %s", fish_name);
    sprintf(desc_buf, "A %s lies here.", fish_name);
    free_string(fish_obj->short_descr);
    free_string(fish_obj->description);
    fish_obj->short_descr = str_dup(short_buf);
    fish_obj->description = str_dup(desc_buf);
    ...
    if (fish_quality == 0)
        send_to_char("You catch something, but it's just junk!\n\r", ch);
    else if (fish_quality <= 3)
        send_to_char("You catch a small fish!\n\r", ch);
    else if (fish_quality <= 6)
        send_to_char("You catch a decent fish!\n\r", ch);
    else if (fish_quality <= 8)
        send_to_char("You catch a large fish!\n\r", ch);
    else
        send_to_char("You catch an amazing fish!\n\r", ch);

    /* Remove bait if used */
    if (bait != NULL)
    {
        extract_obj(bait);
        send_to_char("You use up your bait.\n\r", ch);
    }

    /* Send message to room */
    act("$n catches something while fishing.", ch, NULL, NULL, TO_ROOM);
}
...
void do_butcher(CHAR_DATA *ch, char *argument)
{
    ...
    /* Check if it's a corpse or fish */
    if (obj->pIndexData->vnum == OBJ_VNUM_CORPSE_NPC)
    {
        vnum = OBJ_VNUM_STEAK;
    }
    else if (obj->pIndexData->vnum == OBJ_VNUM_FISH)
    {
        vnum = OBJ_VNUM_FILET;
    }
    else
    {
        send_to_char("You can only butcher corpses or fish.\n\r", ch);
        return;
    }
    
    /* Get the object index for the new item */
    obj_index = get_obj_index(vnum);
    if (obj_index == NULL)
    {
        send_to_char("The butchering template doesn't exist.\n\r", ch);
        return;
    }
    
    /* Create the new item */
    new_obj = create_object(obj_index, 0);
    
    /* Set it as poisoned (raw) */
    new_obj->value[3] = 1; /* 'yes' = poisoned */
    
    /* Put it in the character's inventory */
    ...
    /* Send messages */
    if (vnum == OBJ_VNUM_STEAK)
    {
        send_to_char("You butcher the corpse and get some raw steaks.\n\r", ch);
        act("$n butchers a corpse and gets some raw steaks.", ch, NULL, NULL, TO_ROOM);
    }
    else
    {
        send_to_char("You butcher the fish and get some raw filets.\n\r", ch);
        act("$n butchers a fish and gets some raw filets.", ch, NULL, NULL, TO_ROOM);
    }
}
...
void do_cook(CHAR_DATA *ch, char *argument)
{
    ...
    if (campfire == NULL)
    {
        send_to_char("You need a campfire to cook.\n\r", ch);
        return;
    }
    
    /* Find the item to cook */
    obj = get_obj_carry(ch, arg, ch);
    if (obj == NULL)
    {
        send_to_char("You don't have that.\n\r", ch);
    ...
    /* Check if it's a steak or filet */
    if (obj->pIndexData->vnum != OBJ_VNUM_STEAK && obj->pIndexData->vnum != OBJ_VNUM_FILET)
    {
        send_to_char("You can only cook steaks or filets.\n\r", ch);
        return;
    }
    
    /* Check if it's already cooked (not poisoned) */
    if (obj->value[3] == 0)
    {
        send_to_char("That's already cooked.\n\r", ch);
        return;
    }
    
    /* Cook the item - set value[3] to 0 (not poisoned) */
    obj->value[3] = 0;
    
    /* Update the short description and description to show it's cooked */
    if (obj->pIndexData->vnum == OBJ_VNUM_STEAK)
    {
        sprintf(new_short, "cooked %s", obj->short_descr);
        sprintf(new_desc, "A %s lies here.", new_short);
    }
    else /* OBJ_VNUM_FILET */
    {
        sprintf(new_short, "cooked %s", obj->short_descr);
    ...
    /* Update the object descriptions */
    free_string(obj->short_descr);
    free_string(obj->description);
    obj->short_descr = str_dup(new_short);
    obj->description = str_dup(new_desc);
    
    /* Send messages */
    if (obj->pIndexData->vnum == OBJ_VNUM_STEAK)
    {
        send_to_char("You cook the steaks over the campfire.\n\r", ch);
        act("$n cooks some steaks over the campfire.", ch, NULL, NULL, TO_ROOM);
    }
    else
    {
        send_to_char("You cook the filets over the campfire.\n\r", ch);
        act("$n cooks some filets over the campfire.", ch, NULL, NULL, TO_ROOM);
    }
}
