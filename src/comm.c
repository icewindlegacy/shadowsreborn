...
int main (int argc, char **argv)
{
    ...
    boot_db ();
    mud_logf ("ROM is ready to rock on port %d (%s).", port, mud_ipaddress);
...
int init_socket (int port)
{
    ...
    mud_logf("set address to %s", mud_ipaddress);
...
void game_loop_mac_msdos (void)
{
	...
	if (!mud_ansiprompt)
	{
    	...
    	write_to_buffer (&dcon, "Does your terminal support color? ", 0);
...
#define FNDELAY O_NDELAY
	...
	if (!mud_ansiprompt)
	{
		...
		if ( help_greeting[0] == '.' )
	    	...
	    	send_to_desc ("Does your terminal support color? ", dnew);
    ...
    if (!merc_down)
    {
        ...
        else if (fPrompt && d->connected == CON_PLAYING)
        {
            ...
            if ((victim = ch->fighting) != NULL && can_see (ch, victim))
            {
                ...
                if (percent >= 100)
                    sprintf (wound, "{Wis in excellent condition.{x");
                else if (percent >= 90)
                    sprintf (wound, "{yhas a few scratches.{x");
                else if (percent >= 75)
                    sprintf (wound, "{yhas some small wounds and bruises.{x");
                else if (percent >= 50)
                    sprintf (wound, "{Yhas quite a few wounds.{x");
                else if (percent >= 30)
                    sprintf (wound,
                             "{Bhas some big nasty wounds and scratches.{x");
                else if (percent >= 15)
                    sprintf (wound, "{rlooks pretty hurt.{x");
                else if (percent >= 0)
                    sprintf (wound, "{Ris in awful condition.{x");
                else    
                    sprintf (wound, "{Ris bleeding to death.{x");
     ...
     while (*str != '\0')
     {
         ...
         switch (*str)
         {
             ...
             case 'O':
                 ...
                 break;
             case 'b':
                 /* Graphical battle damage prompt */
                 {
                     CHAR_DATA *victim;
                     char wound[50];
                     int percent;
                 ...
                 break;
             case 'B':
                 /* Percentage battle damage prompt */
                 {
                     CHAR_DATA *victim;
                     int percent;
                 ...
                 /* Current Questpoints */
         case 'q':
         sprintf(buf2, "%-6d", ch->pcdata->quest_curr);
         i = buf2;
         break;
     /* Show Quest Mob or Quest Object -Koqlb */
        case 'I':
         ...
         sprintf(buf2, "%s", questinfo->short_descr);
              ...
              {
         sprintf(buf2, "%s", ch->pcdata->questshort);
          }
         else
         {
         sprintf(buf2, "None");
         ...
         i = buf2;
         break;
     /* Time left for current quest */
        case 'T':
         if (ch->pcdata->countdown > 0)
                {
         sprintf(buf2, "%-2d{cm{x", ch->pcdata->countdown);
                }
         else
         {
         sprintf(buf2, "N/A ");
