...
void say_spell (CHAR_DATA * ch, int sn)
{
    ...
    snprintf (buf2, sizeof(buf2), "$n utters the words, '%.4583s'.", buf);
                     ...
                     OBJ_DATA * obj)
{
    ...
    /* Humans get +3 level bonus when using magical items */
    if (!IS_NPC(ch) && ch->faction == FACTION_HUMAN)
    {
        level += 3;
    }
                           ...
                           int target)
{
    ...
    send_to_char ("Om's lightning strikes your foes!\n\r", ch);
    act ("$n calls Om's lightning to strike $s foes!",
                         ...
                         int target)
{
    ...
    water = UMIN (level * (weather_info[ch->in_room->sector_type].sky >= SKY_RAINING ? 4 : 2),
                        ...
                        int target)
{
    ...
    if (IS_GOOD (victim))
    {
        act ("Om protects $N.", ch, NULL, victim, TO_ROOM);
        return;
    }
                       ...
                       int target)
{
    ...
    for (ich = ch->in_room->people; ich != NULL; ich = ich->next_in_room)
    {
        ...
        STR_REMOVE_BIT (ich->affected_by, AFF_HIDE);
        STR_REMOVE_BIT (ich->affected_by, AFF_INVISIBLE);
        STR_REMOVE_BIT (ich->affected_by, AFF_SNEAK);
                             ...
                             int target)
{
    ...
    if (saves_spell (level, victim, DAM_LIGHTNING))
    {
        ...
        damage (ch, victim, dam / 2, sn, DAM_LIGHTNING, TRUE, FALSE);
    }
    else
    {
...
void spell_regular_shots(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
    ...
    if ( saves_spell( level, victim, DAM_BASH) )
        dam /= 2;
    damage( ch, victim, dam, sn, DAM_BASH ,TRUE, FALSE);
    return;
}
/* Scribe skill - allows PCs to scribe up to 3 spells on a single scroll
 * Re-worked by Sadiq, adapted for ROM by AI Assistant */
void do_scribe( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch) && ch->mana < mana )
    {
     ...
     {
        send_to_char( "That scroll already contains as much magic as it can hold.\n\r", ch);
        return;
     }

     if ( !IS_NPC(ch) && number_percent( ) > get_skill(ch, gsn_scribe) )
     {
       send_to_char("{MThe magic surges out of control and destroys the scroll!{x\n\r", ch);
    ...
    if ( scroll->value[1] == -1 )
     ...
     sprintf(buf1, "a magically inscribed scroll" );
     free_string(scroll->short_descr);
     scroll->short_descr = str_dup( buf1 );
     sprintf(buf2, "A magically inscribed scroll lies in the dust." );
     free_string(scroll->description);
     scroll->description = str_dup(buf2);
     sprintf(buf3, "scroll scribing scribed inscribed %s", skill_table[sn].name);
     free_string(scroll->name);
     scroll->name = str_dup(buf3);

     act( "{M$n magically scribes a scroll.{x",   ch, NULL, NULL, TO_ROOM );
     act( "{MYou magically scribe $p.{x",   ch, scroll, NULL, TO_CHAR );
    ...
    if ( scroll->value[2] == -1 )
     {
      ...
      scroll->value[2] = sn;
      sprintf(buf1, "{MYou imbue the scroll with %s.{x\n\r", skill_table[sn].name);
