 ...
 * Defined in db.c and declared in merc.h
 */
...
void fwrite_obj(CHAR_DATA *ch, OBJ_DATA *obj, FILE *fp, int iNest, bool recurse_next);
...
void save_char_obj (CHAR_DATA * ch)
{
    ...
    else
    {
        ...
        if (ch->carrying != NULL)
            fwrite_obj(ch, ch->carrying, fp, 0, TRUE);  /* ✅ Updated line */
...
void fwrite_char (CHAR_DATA * ch, FILE * fp)
{
    ...
    if (ch->pcdata->leader >= 0)
        fprintf (fp, "Lead %d\n", ch->pcdata->leader);
    if (ch->pcdata->petitioning >= 0)
        fprintf (fp, "Peti %d\n", ch->pcdata->petitioning);
    fprintf (fp, "Fact %d\n", ch->faction);
    fprintf (fp, "Sex  %d\n", ch->sex);
    fprintf (fp, "Cla  %d\n", ch->class);
    fprintf (fp, "Levl %d\n", ch->level);
...
void fwrite_mount (CHAR_DATA * pet, FILE * fp)
{
    ...
    if (pet->act != pet->pIndexData->act)
        fprintf(fp, "Act  %s\n", print_flags(pet->act));
    if (pet->affected_by[0] != pet->pIndexData->affected_by[0])
        fprintf(fp, "AfBy %s\n", print_flags(pet->affected_by[0]));
    if (pet->comm != 0)
        fprintf(fp, "Comm %s\n", print_flags(pet->comm));
...

{
    EXTRA_DESCR_DATA *ed;
    AFFECT_DATA *paf;
/* Save sibling objects only if recursion is allowed */
if (recurse_next && obj->next_content != NULL)
    fwrite_obj(ch, obj->next_content, fp, iNest, recurse_next);
    ...
    switch (obj->item_type)
    {
        ...
        case ITEM_MANUAL:
            if (obj->value[1] > 0)
            {
                fprintf (fp, "Skill 1 '%s'\n",
                         skill_table[obj->value[1]].name);
            }

            if (obj->value[2] > 0)
            {
                fprintf (fp, "Skill 2 '%s'\n",
                         skill_table[obj->value[2]].name);
            }
...
void fwrite_obj_compat(CHAR_DATA *ch, OBJ_DATA *obj, FILE *fp, int iNest)
{
    fwrite_obj(ch, obj, fp, iNest, TRUE);
}

void fwrite_obj_ground(OBJ_DATA *obj, FILE *fp, int iNest)
{
    EXTRA_DESCR_DATA *ed;
    AFFECT_DATA *paf;
...
bool load_char_obj (DESCRIPTOR_DATA * d, char *name)
{
    ...
    ch->pcdata->leader = -1;     /* Not a clan leader by default */
    ch->pcdata->petitioning = -1; /* Not petitioning any clan */
    ...
    if (found)
    {
        ...
        /* Always recalculate faction from race to ensure it's correct */
        ch->faction = get_faction_by_race(ch->race);

        ch->size = pc_race_table[ch->race].size;
        /* Felar use claws instead of punch */
        if (ch->race == race_lookup ("felar"))  /* felar is race 6 */
            ch->dam_type = 5;        /* claw */
        else
    ...
    for (;;)
    {
        ...
        switch (UPPER (word[0]))
        {
            ...
            case 'E':
                ...
                break;
            case 'F':
                KEY ("Fact", ch->faction, fread_number (fp));
                //KEY ("Forget", ch->pcdata->forget[0], fread_number (fp));
                break;

            case 'G':
                KEY ("Gold", ch->gold, fread_number (fp));
                KEY ("Gold_bank", ch->pcdata->gold_bank, (long)fread_number (fp));
            ...
            case 'L':
                ...
                KEY ("Lead", ch->pcdata->leader, fread_number (fp));
            ...
            case 'P':
                ...
                KEY ("Peti", ch->pcdata->petitioning, fread_number (fp));
		...
		ch->pkattempt	= fread_number( fp );
		ch->mkill	= fread_number( fp );
		ch->mdeath	= fread_number( fp );
		/* Check if there are arena stats (backwards compatibility) */
		if (!feof(fp))
		{
		    ...
		    {
			ungetc(letter, fp);
			ch->arenakill	= fread_number( fp );
			ch->arenadeath	= fread_number( fp );
		    }
		    else
		    {
			/* Old format without arena stats */
			ch->arenakill = 0;
			ch->arenadeath = 0;
		    }
		}
		...
		break;
            ...
            case 'S':
                ...
                KEY ("Gold_bank", ch->pcdata->gold_bank, (long)fread_number (fp));
                KEY ("Silv_bank", ch->pcdata->silver_bank, (long)fread_number (fp));
                KEY ("Shares", ch->pcdata->shares, fread_number (fp));
                KEY ("Share_level", ch->pcdata->share_level, fread_number (fp));
                KEY ("Shares_level", ch->pcdata->share_level, fread_number (fp));
    ...
    for (;;)
    {
        ...
        switch (UPPER (word[0]))
        {
            ...
            case 'A':
	    		...
	    		if ( ch->version < 6 )
	    		{
	    	    	KEYF( "AfBy", pet->affected_by, aff_convert_fread_flag(
	    	    							fread_flag( fp ) ), AFF_FLAGS );
	    	    }
	    	    else
	    	    {
...
void fread_mount (CHAR_DATA * ch, FILE * fp )
{
    ...
    for ( ; ; )
    {
    	...
    	case 'A':
            ...
            {
                pet->affected_by[0] = fread_flag(fp);
                fMatch = TRUE;
            }
    	    KEY( "Alig",	pet->alignment,		fread_number(fp));
...
void fread_obj (CHAR_DATA * ch, FILE * fp)
{
    ...
    /* Ensure qcost is initialized for all loaded objects */
    obj->qcost = 0;
    ...
    {
        bugf("fread_obj: Invalid initial nest level %d for obj (unknown)", iNest);
        iNest = 0;
    }
        ...
        switch (UPPER (word[0]))
        {
            ...
            case 'E':
                ...
                if (!str_cmp (word, "End"))
                {
                    ...
                    else
                    {
                        ...
                        /* --- SANITY CHECK: prevent recursive containment --- */
                        if (obj->contains == obj || (obj->in_obj && obj->in_obj == obj))
                        {
                            bugf("fread_obj: Recursive containment detected for vnum %d",
                        ...
                        /* --- DEBUG LOGGING: copyover load tracking --- */
                     /*   if (!ch && obj->in_room)
                        {
                            bugf("CopyoverLoad: %s (vnum %d) -> room %d nest %d",
                        ...
                        {
                            bugf("CopyoverLoad: %s (vnum %d) -> player %s nest %d",
                                 obj->short_descr ? obj->short_descr : "(null)",
                                 obj->pIndexData ? obj->pIndexData->vnum : -1,
                                 ch->name,
                                 iNest);
            ...
            case 'N':
                ...
                if (!str_cmp (word, "Nest"))
                {
                    ...
                    /* --- SANITY CHECK: nest level --- */
                    if (iNest < 0 || iNest >= MAX_NEST)
                    {
                        bugf("Fread_obj: bad nest %d (vnum %d)", iNest,
                             obj->pIndexData ? obj->pIndexData->vnum : -1);
                        iNest = 0;
                    }
                    else
