...
REDIT (redit_oreset)
{
    ...
    if ((to_mob = get_char_room (ch, arg2)) != NULL)
    {
        ...
        /*
         * Can't load into same position (except inventory which allows multiple items).
         */
...
void show_obj_values (CHAR_DATA * ch, OBJ_INDEX_DATA * obj)
{
    ...
    switch (obj->item_type)
    {
        ...
        case ITEM_COMM:
            sprintf (buf,
                     "[v0] Comm1 frequency: [%d]\n\r"
                     "[v1] Comm2 frequency: [%d]\n\r"
                     "[v2] Comm3 frequency: [%d]\n\r"
                     "[v3] Comm4 frequency: [%d]\n\r"
                     "[v4] Charges remaining: [%d]\n\r",
...
/* It somehow fixed a bug in showing scroll/pill/potions too ?! */
        ...
        case ITEM_GUN:
            sprintf (buf, "[v0] Damage:         [%d]\n\r", obj->value[0]);
            send_to_char (buf, ch);
            sprintf (buf, "[v1] Max ammo:       [%d]\n\r", obj->value[1]);
            send_to_char (buf, ch);
            sprintf (buf, "[v2] Current ammo:   [%d]\n\r", obj->value[2]);
            send_to_char (buf, ch);
            sprintf (buf, "[v3] Spell:          %s\n\r",
                     (obj->value[3] >= 0 && obj->value[3] < MAX_SKILL && skill_table[obj->value[3]].name) 
                     ? skill_table[obj->value[3]].name : "none");
            send_to_char (buf, ch);
            sprintf (buf, "[v4] Gun flags:      %s\n\r",
                     gun_bit_name(obj->value[4]));
            send_to_char (buf, ch);
            break;
        ...
        case ITEM_AMMO:
            sprintf (buf, "[v0] Damage:         [%d]\n\r", obj->value[0]);
            send_to_char (buf, ch);
            sprintf (buf, "[v1] Ammo count:     [%d]\n\r", obj->value[1]);
            send_to_char (buf, ch);
            sprintf (buf, "[v2] Unused:         [%d]\n\r", obj->value[2]);
            send_to_char (buf, ch);
            sprintf (buf, "[v3] Spell:          %s\n\r",
                     (obj->value[3] >= 0 && obj->value[3] < MAX_SKILL && skill_table[obj->value[3]].name) 
                     ? skill_table[obj->value[3]].name : "none");
            send_to_char (buf, ch);
            sprintf (buf, "[v4] Ammo flags:     %s\n\r",
                     gun_bit_name(obj->value[4]));
            send_to_char (buf, ch);
            break;

        case ITEM_BAIT:
            sprintf (buf, "[v0] Quality:        [%d]\n\r", obj->value[0]);
            send_to_char (buf, ch);
            break;
        ...
        case ITEM_TOBACCO:
            sprintf (buf,
                     ...
                     "[v3] Spell 3: [%d] %s\n\r",
                     obj->value[0], 
                     obj->value[1], obj->value[1] > 0 && obj->value[1] < MAX_SKILL ? skill_table[obj->value[1]].name : "None",
                     obj->value[2], obj->value[2] > 0 && obj->value[2] < MAX_SKILL ? skill_table[obj->value[2]].name : "None",
                     obj->value[3], obj->value[3] > 0 && obj->value[3] < MAX_SKILL ? skill_table[obj->value[3]].name : "None");
            send_to_char (buf, ch);
            break;
                     ...
                     char *argument)
{
    switch (pObj->item_type)
    {
        ...
        case ITEM_MANUAL:
            switch (value_num)
            {
                default:
                    ...
                    return FALSE;
                case 0:
                    send_to_char ("SKILL LEVEL SET.\n\r\n\r", ch);
                    pObj->value[0] = atoi (argument);
                    break;
                case 1:
                    send_to_char ("SKILL TYPE 1 SET.\n\r\n\r", ch);
                    pObj->value[1] = skill_lookup (argument);
                    break;
                case 2:
                    send_to_char ("SKILL TYPE 2 SET.\n\r\n\r", ch);
                    pObj->value[2] = skill_lookup (argument);
                    break;
                case 3:
                    send_to_char ("SKILL TYPE 3 SET.\n\r\n\r", ch);
                    pObj->value[3] = skill_lookup (argument);
                    break;
                case 4:
                    send_to_char ("SKILL TYPE 4 SET.\n\r\n\r", ch);
                    pObj->value[4] = skill_lookup (argument);
                    break;
            }
            break;
        ...
        case ITEM_COMM:
            ...
            {
                default:
                    send_to_char("Only values 0-4 are valid.\n\r", ch);
                    return FALSE;
                case 0:
                case 1:
                case 2:
                case 3:
                    pObj->value[value_num] = URANGE(0, atoi(argument), COMMSTONE_MAX_FREQUENCY);
                    send_to_char("Frequency set.\n\r\n\r", ch);
                    break;
                case 4:
                    pObj->value[4] = UMAX(0, atoi(argument));
...
/* WEAPONS changed in ROM */
        ...
        case ITEM_GUN:
            switch (value_num)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    send_to_char ("CURRENT AMMO SET.\n\r\n\r", ch);
                    pObj->value[2] = atoi (argument);
                    break;
                case 3:
                    send_to_char ("GUN SPELL SET.\n\r\n\r", ch);
                    pObj->value[3] = skill_lookup (argument);
                    break;
                case 4:
                    send_to_char ("GUN FLAGS TOGGLED.\n\r\n\r", ch);
                    ALT_FLAGVALUE_TOGGLE (pObj->value[4], gun_flags,
                                          argument);
                    break;
            }
            break;
        ...
        case ITEM_AMMO:
            switch (value_num)
            {
                default:
                    ...
                    return FALSE;
                case 0:
                    send_to_char ("AMMO DAMAGE SET.\n\r\n\r", ch);
                    pObj->value[0] = atoi (argument);
                    break;
                case 1:
                    send_to_char ("AMMO COUNT SET.\n\r\n\r", ch);
                    pObj->value[1] = atoi (argument);
                    break;
                case 2:
                    send_to_char ("AMMO UNUSED SET.\n\r\n\r", ch);
                    pObj->value[2] = atoi (argument);
                    break;
                case 3:
                    send_to_char ("AMMO SPELL SET.\n\r\n\r", ch);
                    pObj->value[3] = skill_lookup (argument);
                    break;
                case 4:
                    send_to_char ("AMMO FLAGS TOGGLED.\n\r\n\r", ch);
                    ALT_FLAGVALUE_TOGGLE (pObj->value[4], gun_flags,
                                          argument);
                    break;
            }
        ...
        case ITEM_BAIT:
            ...
            {
                default:
                    do_help (ch, "ITEM_BAIT");
                    return FALSE;
                case 0:
                    send_to_char ("BAIT QUALITY SET.\n\r\n\r", ch);
                    pObj->value[0] = atoi (argument);
                    break;
            }
            break;
        ...
        case ITEM_FISHING_POLE:
            ...
            {
                default:
                    do_help (ch, "ITEM_FISHING_POLE");
                    return FALSE;
                case 0:
                    send_to_char ("FISHING POLE QUALITY SET.\n\r\n\r", ch);
                    pObj->value[0] = atoi (argument);
                    break;
            }
            break;
        ...
        case ITEM_TOBACCO:
            switch (value_num)
            {
                ...
                case 0:
                    ...
                    break;
                case 1:
                    send_to_char ("SPELL 1 SET.\n\r\n\r", ch);
                    pObj->value[1] = skill_lookup (argument);
                    break;
                case 2:
                    send_to_char ("SPELL 2 SET.\n\r\n\r", ch);
                    pObj->value[2] = skill_lookup (argument);
                    break;
                case 3:
                    send_to_char ("SPELL 3 SET.\n\r\n\r", ch);
                    pObj->value[3] = skill_lookup (argument);
                    break;
            }
            break;
...
OEDIT (oedit_addapply)
{
    ...
    if (bvector[0] == '\0'
        || (bv = flag_value (bitvector_type[typ].flags, bvector)) == NO_FLAG)
    {
        send_to_char ("Invalid bitvector type.\n\r", ch);
        send_to_char ("Valid bitvector types are:\n\r", ch);
        show_help (ch, bitvector_type[typ].name);
        return FALSE;
    }
...
/* Delete functions by Christopher Feist (from olc_delete.c snippet) */
...
MEDIT (medit_qshop)
{
    ...
    if (command[0] == '\0')
    {
        send_to_char("Syntax:  qshop list\n\r", ch);
        send_to_char("         qshop add <item vnum>\n\r", ch);
        send_to_char("         qshop remove <item vnum>\n\r", ch);
        send_to_char("         qshop clear\n\r", ch);
        return FALSE;
    }

    if (!str_cmp(command, "list"))
    {
        send_to_char("Quest shop inventory:\n\r", ch);
        if (!pMob->pShop)
        {
            send_to_char("  No quest shop assigned.\n\r", ch);
            return FALSE;
        }
        ...
        for (i = 0; i < MAX_TRADE; i++)
        {
            ...
            {
                OBJ_INDEX_DATA *obj_index = get_obj_index(pMob->pShop->buy_type[i]);
                if (obj_index && IS_OBJ_STAT(obj_index, ITEM_QUEST))
                {
                    sprintf(buf, "  %d. %s (qcost: %d)\n\r", 
                            i+1, obj_index->short_descr, QUEST_COST(obj_index));
                    send_to_char(buf, ch);
                }
            }
        }
    ...
    if (!str_cmp(command, "add"))
    {
        if (arg1[0] == '\0' || !is_number(arg1))
        {
            send_to_char("Syntax:  qshop add <item vnum>\n\r", ch);
            return FALSE;
        }

        OBJ_INDEX_DATA *obj_index = get_obj_index(atoi(arg1));
        if (!obj_index)
        {
        ...
        if (!IS_OBJ_STAT(obj_index, ITEM_QUEST))
        {
            send_to_char("That object is not a quest item.\n\r", ch);
            return FALSE;
        }

        if (!pMob->pShop)
        {
            send_to_char("You must assign a shop first (shop assign).\n\r", ch);
            return FALSE;
        }
        ...
        for (i = 0; i < MAX_TRADE; i++)
        {
            if (pMob->pShop->buy_type[i] == 0)
            {
                pMob->pShop->buy_type[i] = obj_index->vnum;
                sprintf(buf, "Added %s to quest shop.\n\r", obj_index->short_descr);
                send_to_char(buf, ch);
                return TRUE;
            }
        }

        send_to_char("Quest shop is full.\n\r", ch);
        return FALSE;
    }
    ...
    if (!str_cmp(command, "remove"))
    {
        if (arg1[0] == '\0' || !is_number(arg1))
        {
            send_to_char("Syntax:  qshop remove <item vnum>\n\r", ch);
            return FALSE;
        }
        ...
        for (i = 0; i < MAX_TRADE; i++)
        {
            if (pMob->pShop->buy_type[i] == atoi(arg1))
            {
                OBJ_INDEX_DATA *obj_index = get_obj_index(atoi(arg1));
                pMob->pShop->buy_type[i] = 0;
                sprintf(buf, "Removed %s from quest shop.\n\r", 
                        obj_index ? obj_index->short_descr : "item");
                send_to_char(buf, ch);
                return TRUE;
            }
        }

        send_to_char("Item not found in quest shop.\n\r", ch);
        return FALSE;
    }
    ...
    if (!str_cmp(command, "clear"))
    {
        ...
        send_to_char("Quest shop cleared.\n\r", ch);
        return TRUE;
    }

    send_to_char("Unknown qshop command.\n\r", ch);
    return FALSE;
}

/* Automob functions - automatically set mob values based on level */
MEDIT( medit_autoset )
{
    MOB_INDEX_DATA *pMob;
    char temp[500];
    ...
    ac_n = 95 - (pMob->level * 6.67) - ((pMob->level/10)*(pMob->level/10));
    ac_x = 95 - (pMob->level * 4.57) - ((pMob->level/10)*(pMob->level/10));
...
MEDIT( medit_autohard )
{
    ...
    ac_n = 88 - (pMob->level * 7.05) - ((pMob->level/10)*(pMob->level/10));
    ac_x = 88 - (pMob->level * 5.02) - ((pMob->level/10)*(pMob->level/10));
    pMob->ac[AC_PIERCE] = ac_n;
    pMob->ac[AC_BASH]   = ac_n;
    pMob->ac[AC_SLASH]  = ac_n;
    pMob->ac[AC_EXOTIC] = ac_x;
...
MEDIT( medit_autoeasy )
{
    ...
    ac_n = 99 - (pMob->level * 6.37) - ((pMob->level/10)*(pMob->level/10));
    ac_x = 99 - (pMob->level * 4.27) - ((pMob->level/10)*(pMob->level/10));
    pMob->ac[AC_PIERCE] = ac_n;
    pMob->ac[AC_BASH]   = ac_n;
    pMob->ac[AC_SLASH]  = ac_n;
    pMob->ac[AC_EXOTIC] = ac_x;
...
/* Autoset functions - automatically set object values based on level */
...
OEDIT( oedit_autoarmor )
{
    ...
    pObj->weight = pObj->level + 1;
    pObj->cost = pObj->level * pObj->level * 2;	
    pObj->value[0] = size;
    pObj->value[1] = size;
    pObj->value[2] = size;
    pObj->value[3] = (size - 1);
...
OEDIT( oedit_timer )
{
    ...
    if (argument[0] == '\0')
    {
        send_to_char( "Syntax:  timer [0-9999]\n\r", ch);
        send_to_char( "Set timer to 0 to disable auto-dissolve.\n\r", ch);
        return FALSE;
    }
    ...
    {
        send_to_char( "Timer must be between 0 and 9999.\n\r", ch);
        return FALSE;
    }

    pObj->timer = timer;
    if (timer == 0)
        send_to_char( "Timer disabled - object will not auto-dissolve.\n\r", ch);
    else
        send_to_char( "Timer set - object will dissolve after %d updates.\n\r", ch);
