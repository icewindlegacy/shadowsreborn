...
void move_char (CHAR_DATA * ch, int door, bool follow)
{
    ...
    /* Drunken walking - by Kohl Desenee */
    if (!IS_NPC(ch))
    {
    ...
    {
        /* Drunk message for invalid exit */
    ...
    if ( MOUNTED(ch) )
    {
        ...
        if (!IS_IMMORTAL(ch) && !mount_success(ch, MOUNTED(ch), FALSE))
        {
    ...
    /* Swimming check for water sectors */
    ...
    if (RIDDEN(ch))
    {
        ...
        /* Move the rider with the mount */
        char_from_room( rch );
        char_to_room( rch, to_room );
        if (!IS_AFFECTED (rch, AFF_SNEAK) && rch->invis_level < LEVEL_HERO)
            act ("$n arrives.", rch, NULL, NULL, TO_ROOM);

        /* If rider is mounted, they already successfully mounted - allow movement */
        /* Only check again if rider is not an immortal (immortals always succeed) */
        if (!IS_IMMORTAL(rch) && !mount_success(rch, ch, FALSE))
        {
...
void do_hide (CHAR_DATA * ch, char *argument)
{
    ...
    {
        STR_SET_BIT (ch->affected_by, AFF_HIDE);
...
void do_recall (CHAR_DATA * ch, char *argument)
{
    ...
    /* Check if they want to recall to clan (only if they're in a clan) */
    if (!str_cmp (arg, "clan"))
    {
        if (!IS_CLAN(ch))
        {
...
void do_recall_clan(CHAR_DATA * ch, char *argument)
{
    do_recall(ch, "clan");
    return;
}
